(() => {
    ...
    function calcDividers() {
        ...
        if (divInput.length) createResult(".dividers__body", "dividers__result", `Число ${divInput} можно делить на:` + " " + result);
    }
    function setWordCount(field) {
        const wordCount = field.match(/(\p{L}{2,})/gu);
        if (null !== wordCount) createResult(".search-text__body", "search-text__value", `Количество слов в данной строке: ${wordCount.length}`); else createResult(".search-text__body", "search-text__num", "Я считаю только слова и символы");
    }
    function setSymbolCount(field) {
        const symbolCount = field.match(/\S/g);
        if (null !== symbolCount) createResult(".search-text__body", "search-text__value", `Количество символов в данной строке: ${symbolCount.length}`);
    }
